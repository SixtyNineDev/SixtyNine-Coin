...
#include "zSixtyNinecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zSXT ought to be enough for anybody." - Bill Gates, 2017
    ui->zSXTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSXTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzSXT->setEnabled(false);
        ui->pushButtonMintzSXT->setToolTip(tr("zSXT is currently disabled due to maintenance."));

        ui->pushButtonSpendzSXT->setEnabled(false);
        ui->pushButtonSpendzSXT->setToolTip(tr("zSXT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSXTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSXT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSXT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSXT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSXT in ") + 
...
void PrivacyDialog::on_pushButtonSpendzSXT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSXT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zSXT
        sendzSXT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSXT
    sendzSXT();
}

void PrivacyDialog::on_pushButtonZSXTControl_clicked()
{
    ZSXTControlDialog* zSXTControl = new ZSXTControlDialog(this);
    zSXTControl->setModel(walletModel);
    zSXTControl->exec();
}

void PrivacyDialog::setZSXTControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzSXTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSXT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid SixtyNine Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zSXTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSXT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " SXT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSXT</b>";
    ...
    // use mints from zSXT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZSXTControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZSXTControlDialog::GetSelectedMints();
    }
    ...
    // Clear zSXT selector in case it was used
    ZSXTControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSXT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " SXT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSXT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSXT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSXT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSXT "));
    ui->labelzSXTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzSXTSyncStatus->setVisible(fShow);
}
