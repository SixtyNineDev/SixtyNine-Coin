...
#include "ui_zSixtyNinecontroldialog.h"
...
std::list<std::string> ZSXTControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZSXTControlDialog::listMints;

ZSXTControlDialog::ZSXTControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZSXTControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZSXTControlDialog::~ZSXTControlDialog()
{
    delete ui;
}

void ZSXTControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSXTControlDialog::updateList()
{
...
void ZSXTControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSXTControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZSXT_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZSXTControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZSXTControlDialog::GetSelectedMints()
{
...
void ZSXTControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
