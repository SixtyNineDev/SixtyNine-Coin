...
#if defined(NDEBUG)
#error "SixtyNine cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 60 * 1; // * 1  for lauch, 1hrs;
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zSXT spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zSXT spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zSXT spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue( int nHeight )
{
	...
	if ( nHeight < 1 ){
		nSubsidy = 69696 * COIN;
	} else if ( nHeight <= 8868 ) {
		nSubsidy = 2 * COIN;
	} else if ( nHeight <= 17696 ) {
		nSubsidy = 696 * COIN;
	} else if ( nHeight <= 18968 ) {
		nSubsidy = 169 * COIN;
	} else if ( nHeight <= 27696 ) {
		nSubsidy = 69 * COIN;
	} else if ( nHeight <= 35696 ) {
		nSubsidy = 50 * COIN;
	} else if ( nHeight <= 69695 ) {
		nSubsidy = 75 * COIN;
	} else if ( nHeight <= 139695 ) {
		nSubsidy = 100 * COIN;
	} else if ( nHeight <= 696968 ) {
		nSubsidy = 150 * COIN;
	} else if ( nHeight <= 1696968 ) {
		nSubsidy = 250 * COIN;
	} else if ( nHeight <= 2969695 ) {
		nSubsidy = 150 * COIN;
	} else if ( nHeight > 2969695 ) {
		nSubsidy = 100 * COIN;
	} else {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= 200) {
        return 0;
    } else if (nHeight < 8869 && nHeight > 200) {
        return 0;
    } else if (nHeight < 17697 && nHeight >= 8869) {
	return blockValue * 0.99;
    } else if (nHeight < 18969 && nHeight >= 17697) {
	return blockValue * 0.75;
    } else if (nHeight < 35697 && nHeight >= 18969) {
	return blockValue * 0.7;
    } else if (nHeight < 69696 && nHeight >= 35697) {
	return blockValue * 0.7;
    } else if (nHeight < 139696 && nHeight >= 69696) {
	return blockValue * 0.75;
    } else if (nHeight < 696969 && nHeight >= 139696) {
	return blockValue * 0.75;
    } else if (nHeight < 1696969 && nHeight >= 696969) {
	return blockValue * 0.75;
    } else if (nHeight < 2969697 && nHeight >= 1696969) {
	return blockValue * 0.75;
    } else if (nHeight > 2969696) {
	return blockValue * 0.75;
    } else {
        return blockValue * 0.75;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SixtyNine
...
{
    RenameThread("SixtyNine-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZSXTMinted()
{
...
void RecalculateZSXTSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zSXT supply
...
bool RecalculateSXTSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zSXT with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zSXTSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert SXT to zSXT
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zSXT spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zSXT serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zSXT mints and %d zSXT spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SixtyNine: We use certain sporks during IBD, so check to see if they are
